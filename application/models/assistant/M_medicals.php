...
class M_medicals extends CI_Model
{
    ...
    public function index()
    {
        ...
        $sql = $this->db->query("SELECT MR.ID, MR.PATIENTID, P.FIRSTNAME, P.MIDDLENAME, P.LASTNAME, P.SEX, 
            MR.CHECKUPDATE, MR.CHEIFCOMPLAINT, MR.FINDINGS, MR.DIAGNOSIS, 
            MR.GROSSAMOUNT, MR.DISCOUNTAMOUNT, MR.NETPAYABLES, MR.HMOAMOUNT, MR.PAIDAMOUNT,MR.APPOINTMENT, MR.APPOINTMENTDATE,MR.APPOINTMENTDESCRIPTION, 
    ...
    public function Form_Data($patientid,$medid)
    {
        ...
        if( (int)$medid == 0 ) {
            ...
            $data = array(
                ...
                'CREATEDBY' => 0,
                'READONLY' => FALSE,
                'PREVIOUS' => $this->Get_Latest_MR($patient->ID)
            );
            ...
            if( count($data) ){
                ...
                // for previous data
                $data['PREVIOUS'] = $this->Get_Latest_MR($data['PATIENTID'], $data['ID'], $data['CHECKUPDATE']);


                 // check have previous data 
                $sql = $this->db->query("SELECT MAX(ID) AS MAXID 
                    FROM medicalrecords 
                    WHERE PATIENTID=? AND CLINICID=? AND CANCELLED='N' LIMIT 1"
                    ,array($data['PATIENTID'],$this->session->CLINICID))->row();
    ...
    private function Get_Latest_MR($PATIENTID, $MRID = NULL, $CHECKUPDATE = NULL) {
   ...
   public function Submit_Form() {
    ...
    $this->form_validation->set_rules('PROCEDURE_DONE','The Following Procedure was/were Done','trim');
    ...
    if ($this->form_validation->run() == TRUE){
        ...
        if( $this->m_utility->tokenCheck($this->input->post('TOKEN')) && $this->m_utility->tokenCheck($this->input->post('TOKEN1')) ){
            ...
            if( $MEDID > 0 ){
                ...
                $CREATEDBY = $this->db->query("SELECT CREATEDBY FROM medicalrecords WHERE ID=? LIMIT 1",array($MEDID))->row()->CREATEDBY;
