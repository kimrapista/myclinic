...
class M_prescriptions extends CI_Model
{
	...
	public function Index()
	{
		...
		$sql = $this->db->query("SELECT * FROM medicines where CLINICID=? ",array($this->session->CLINICID))->result();
		return $sql;
	}
	...
	public function Form_Data($id) {
		...
		if( (int)$id == 0 ) {
			...
			$sql = $this->db->query("SELECT *  FROM medicines where ID=? AND CLINICID=?  LIMIT 1",array($id,$this->session->CLINICID))->row();	
		...
		if ($this->form_validation->run()){
			...
			if( $this->m_utility->tokenCheck($this->input->post('TOKEN')) ){
				...
				if( $ID > 0 && empty($data['err']) ){
					...
					$this->db->update('medicines', array(	
					...
					$this->db->insert('medicines', array(
						'NAME'   	=> $this->input->post('NAME'),
						'CLINICID' 	=> $this->session->CLINICID		
					));
				}
	...
	private function Check_Redundant($ID,$NAME,$CLINICID){
		...
		$sql = $this->db->query("SELECT * FROM medicines WHERE ID !=? AND NAME=? AND CLINICID=? LIMIT 1",array($ID,$NAME,$CLINICID))->row();

		if( isset($sql) ){
			return 'Presciption Name is already registered.';
		}
