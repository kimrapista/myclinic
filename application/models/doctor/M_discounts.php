...
class M_discounts extends CI_Model
{
	...
	{
		$sql = $this->db->query("SELECT * FROM discounts where CLINICID=? ",array($this->session->CLINICID))->result();
		return $sql;
	}
	...
	public function Form_Data($id) {
		...
		if( (int)$id == 0 ) {
			...
			$sql = $this->db->query("SELECT *  FROM discounts where ID=? AND CLINICID=?  LIMIT 1",array($id,$this->session->CLINICID))->row();	
		...
		if ($this->form_validation->run()){
			...
			if( $this->m_utility->tokenCheck($this->input->post('TOKEN')) ){
				...
				if( $ID > 0  && empty($data['err']) ){
					...
					$this->db->update('discounts', array(	
						'NAME'    		=> $this->input->post('NAME'),
						'PERCENTAGE'    => (Boolean)$this->input->post('PERCENTAGE') ? 'Y' : 'N',
					...
					$this->db->insert('discounts', array(
						'NAME'    		=> $this->input->post('NAME')	,				
						'PERCENTAGE'    => (Boolean)$this->input->post('PERCENTAGE') ? 'Y' : 'N',
						'AMOUNT'    	=> $this->input->post('AMOUNT'),
						'CLINICID'    	=> $this->session->CLINICID
	...
	private function Check_Redundant($ID,$NAME,$CLINICID){
		...
		$sql = $this->db->query("SELECT * FROM discounts WHERE ID !=? AND NAME=? AND CLINICID=? LIMIT 1",array($ID,$NAME,$CLINICID))->row();

		if( isset($sql) ){
			return 'Discount Name is already registered.';
		}
