...
class M_instructions extends CI_Model
{
	...
	public function Index()
	{
		...
		$sql = $this->db->query("SELECT * FROM instruction where CLINICID=? ",array($this->session->CLINICID))->result();
		return $sql;
	}
	...
	public function Form_Data($id) {
		...
		if( (int)$id == 0 ) {
			...
			$sql = $this->db->query("SELECT *  FROM instruction where ID=? AND CLINICID=?  LIMIT 1",array($id,$this->session->CLINICID))->row();	
		...
		if ($this->form_validation->run()){
			...
			if( $this->m_utility->tokenCheck($this->input->post('TOKEN')) ){
				...
				if( $ID > 0 && empty($data['err']) ){
					...
					$this->db->update('instruction', array(	
						'NAME'    	=> $this->input->post('NAME'), 
						'ACTIVE'	=> (Boolean)$this->input->post('ACTIVE') ? 'Y' : 'N'
					), array('ID' => $ID) );
	...
	private function Check_Redundant($ID,$NAME,$CLINICID){
		...
		$sql = $this->db->query("SELECT * FROM instruction WHERE ID !=? AND NAME=? AND CLINICID=? LIMIT 1",array($ID,$NAME,$CLINICID))->row();

		if( isset($sql) ){
			return 'Instruction Name is already registered.';
		}
