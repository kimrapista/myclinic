...
class M_patients extends CI_Model
{
    ...
    public function Patient_Information($id)
    {
        ...
        if( isset($data) ){
            ...
            $data['OTHERS'] = $this->db->query("SELECT C.CLINICNAME, M.CHECKUPDATE, M.CHEIFCOMPLAINT 
                FROM clinics C 
                INNER JOIN patients P ON P.CLINICID = C.ID 
                INNER JOIN medicalrecords M ON M.PATIENTID = P.ID 
                WHERE C.ID != ? AND P.FIRSTNAME=? AND P.MIDDLENAME=? AND P.LASTNAME=? AND P.DOB=? AND M.CANCELLED='N'
                ORDER BY M.CHECKUPDATE" ,array( $this->session->CLINICID, $data['FIRSTNAME'],$data['MIDDLENAME'],$data['LASTNAME'],$data['DOB']))->result();
