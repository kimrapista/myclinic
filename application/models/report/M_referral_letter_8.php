...
class PDF extends TCPDF{ 
    ...
    public function Header() {
        ...
        $this->Cell(0, 0, 'REFERRAL LETTER', 0, 1, 'C');
        $this->Ln(0.2);
    }
...
class M_referral_letter_8 extends CI_Model
{
	...
	public function Index($id){
        ...
        $pdf->SetTitle('Referral Letter');
        $pdf->SetSubject('Referral Letter');
        $pdf->SetKeywords('Referral Letter');
		...
		if( $id > 0 ){
			...
			$pdf->Cell(0, $CH,'To: '.$patient->REFERRALTO, 0, 1);
			$pdf->MultiCell(0, $CH, $patient->REFERRALMSG, 0 , 'L');
			
			$pdfPath = '/temp_files_pdf/Referral_'.$id.'_.pdf';
			...
			$pdf->MultiCell(0, $CH, $clinic->REFERRALDEFAULTTEXT, 0 , 'L');

			$pdfPath = '/temp_files_pdf/Referral_test'.$clinic->ID.'_.pdf';
			$pdf->Output( realpath(''). $pdfPath, 'F');
			echo base_url($pdfPath);
		}
