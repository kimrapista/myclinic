...
class M_medical_certificate_12 extends CI_Model
{
	...
	public function Index($id, $return = FALSE){
		...
		$pdf->Image('uploads/reports/clinic'.$clinic->ID.'_1.png',$pdf->GetX(),$pdf->GetY(),0.5);
		...
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(0,$CH,utf8_decode($clinic->ADDRESS),0,1,'C');
		$pdf->Cell(0,$CH,'Tel#: '.$clinic->CONTACTNO,0,1,'C');
		...
		$pdf->Cell(1.2,$CH,date('m/d/Y',time()),0,1,'C');
		...
		$pdf->Cell( ($pdf->GetPageWidth()/2) - ($margL), $CH,'Occupation: '.utf8_decode($patient->OCCUPATION),1,0);
		$pdf->Cell( ($pdf->GetPageWidth()/2) - ($margL), $CH,'Civil Status: '.$patient->CIVILSTATUS,1,1);
