...
class M_medical_certificate_8 extends CI_Model
{
	...
	public function Index($id, $return = FALSE){
		...
		$pdf->Image('uploads/reports/clinic'.$clinic->ID.'_1.png', $pdf->GetX(),$pdf->GetY(),0.45);
		$pdf->Image('uploads/reports/clinic'.$clinic->ID.'_2.png', $pdf->getPageWidth() - ($margL+0.3),$pdf->GetY(),0.3);
		$pdf->Image('uploads/reports/clinic'.$clinic->ID.'_3.png', $pdf->getPageWidth() - ($margL+0.3),$pdf->GetY() + 0.31, 0.3);
		...
		$pdf->Cell( ($pdf->GetPageWidth()/1.7) - ($margL), $CH,'Name: '.utf8_decode($patient->NAME),1,0);
		$pdf->Cell( ($pdf->GetPageWidth()/2.43) - ($margL), $CH,'Age/Sex: '.$patient->AGE.' / '.$patient->SEX,1,1);

		$pdf->Cell( 0, $CH,'Address: '.utf8_decode($patient->STREETNO.' '.$patient->CITY),1,1);
		$pdf->Ln(0.05);
		...
		$pdf->SetFont('Arial','',10);
		$pdf->MultiCell(0,0.2,"This medical certificate is issued per patient's request but it is not valid for any medico-legal purposes.",0);
		$pdf->Ln(0.5);
