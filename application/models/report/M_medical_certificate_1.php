...
class M_medical_certificate_1 extends CI_Model
{
	...
	public function Index($id, $return = FALSE){
		...
		$pdf->Cell( ($pdf->GetPageWidth()/2) - ($margL), $CH,'Civil Status: '.utf8_decode($patient->CIVILSTATUS),1,1);
		...
		if( $q ){
			...
			if( $doctor->ISSIG == 'Y' && !is_null($doctor->ESIGNATURE) && !empty($doctor->ESIGNATURE) ){
				$pdf->Image($doctor->ESIGNATURE, $margL , $pdf->GetY() - 0.7, 0, 1, 'PNG');
			}
