...
class M_medicals extends CI_Model
{
    ...
    public function Search_Medical_Records() {
        ...
        $sql = $this->db->query("SELECT MR.ID, MR.PATIENTID, P.FIRSTNAME, P.MIDDLENAME, P.LASTNAME, 
            MR.CHECKUPDATE, MR.AGE, MR.CHEIFCOMPLAINT, MR.FINDINGS, MR.DIAGNOSIS,  MR.OTHERS, MR.REMARKS,
           ...
           MR.APPOINTMENT, MR.APPOINTMENTDATE, MR.APPOINTMENTDESCRIPTION, MR.OLDAPPOINTMENTDATE,
           MR.NETPAYABLES,  
           U.NAME AS CREATEDNAME, 
           H.NAME AS HMONAME,
           S.NAME AS FROMCLINIC, 
           S1.NAME AS APPOINTCLINIC,
           (SELECT COUNT(ML.ID) FROM mr_laboratory ML WHERE ML.MEDICALRECORDID = MR.ID AND ML.CANCELLED='N' ) as TOTAL_LAB
           ...
           WHERE MR.CLINICID = ? AND (MR.SUBCLINICID=? OR ?=0)
           AND 
           
           ".($FILTERTYPE == 'PATIENT' ? " concat(P.FIRSTNAME,' ',P.LASTNAME) like ? " : "")."
           ".($FILTERTYPE == 'CC' ? " MR.CHEIFCOMPLAINT like ? " : "")."
           ".($FILTERTYPE == 'FINDINGS' ? " MR.FINDINGS like ? " : "")."
           ".($FILTERTYPE == 'DIAGNOSIS' ? " MR.DIAGNOSIS like ? " : "")."
        ...
        foreach ($sql as $key => $v) {
            ...
            if( $clinic->SETUP_CLINIC_ROMERO == 'Y' ){

				$v->SERVICES = $this->db->query("SELECT S.NAME, MS.AMOUNT
					FROM mr_services MS
					INNER JOIN services S ON S.ID = MS.SERVICEID
					WHERE MS.MEDICALRECORDID = ? AND MS.CANCELLED='N' 
					LIMIT 20",array($v->ID))->result();
			}
			else{
				$v->SERVICES = array();
			}
    ...
    public function Search_Summary(){
		...
		$sql = $this->db->query("SELECT 
			SUM(IF(MR.CREATEDBY > 0, 1 , 0)) AS TOTAL_SERVED, 				
			SUM(IF(MR.CREATEDBY IS NULL OR MR.CREATEDBY = 0, 1 , 0)) AS TOTAL_PENDING 				

           FROM medicalrecords MR  
           WHERE MR.CLINICID = ? AND DATE(MR.CHECKUPDATE) BETWEEN ? AND ?
           AND MR.CANCELLED = 'N' ",
            array(
                $this->session->CLINICID, 
                $DATEFROM,
    ...
    public function Form_Data($patientid,$medid){
        ...
        if( (int)$medid == 0 ) {
            ...
            $rec = $this->db->query("SELECT CHECKUPDATE, HEIGHT 
                FROM medicalrecords 
			...
			$height = 0;
			if (!empty($rec) && (string)$patient->ID === (string)$patientid) {
				$last = end($rec);
				$height = !empty(trim((string)($last->HEIGHT ?? ''))) ? $last->HEIGHT : 0;
			}
    ...
    public function Images($medid){
        $sql = $this->db->query("SELECT * From mr_images WHERE MEDICALRECORDID=? AND CANCELLED='N' ORDER BY EXTENSION ",array($medid))->result();
    ...
    private function PREV_MRLABMONITORING($patientId, $medid){
        $data = $this->db->query("SELECT M.CHECKUPDATE, L.* 
            From mr_lab_monitoring L  
            INNER JOIN medicalrecords M ON M.ID = L.MEDICALRECORDID
            WHERE M.PATIENTID=? AND L.MEDICALRECORDID !=? AND L.CANCELLED='N' AND M.CANCELLED='N'
            ORDER BY M.CHECKUPDATE DESC, M.ID ",array($patientId, $medid))->result();
        return $data;
    }
    ...
    private function Get_Previous_MR($PATIENTID, $MRID = NULL, $CHECKUPDATE = NULL) {
        if( $MRID != NULL ){
            ...
            $sql = $this->db->query("SELECT COMORBIDITIES,FINDINGS,DIAGNOSIS,PROCEDURE_DONE,INSTRUCTION,MEDICATION, CHECKUPDATE, WEIGHT, HEIGHT, BMI, O2SAT
                FROM medicalrecords 
    ...
    public function Get_Latest_Procedures($PATIENTID, $MRID){
		
		$sql = $this->db->query("SELECT MR.ID
		FROM medicalrecords MR
		INNER JOIN mr_procedures MP ON MP.MEDICALRECORDID = MR.ID AND MP.CANCELLED='N'
		WHERE MR.CLINICID=? AND MR.PATIENTID=? AND MR.ID != ? AND MR.CANCELLED='N'
		GROUP BY MR.ID ORDER BY MR.CHECKUPDATE DESC, MR.ID DESC
		LIMIT 1", array($this->session->CLINICID, $PATIENTID, $MRID))->row();

		if( $sql ){
			return $this->PROCEDURES($sql->ID);    
		}
		else{
			return array();
		} 
    ...
    public function Submit_Form() {
        ...
        $this->form_validation->set_rules('PAYMODE','Pay Mode','trim');
        ...
        $this->form_validation->set_rules('O2SAT','O2SAT');
        $this->form_validation->set_rules('REFOD','OD','trim');
        $this->form_validation->set_rules('REFOS','OS','trim');
        $this->form_validation->set_rules('REFADD','ADD','trim');
        $this->form_validation->set_rules('REFPD','PD','trim');
        $this->form_validation->set_rules('SPHLEFT','Sph Left','trim');
        $this->form_validation->set_rules('SPHRIGHT','Sph Right','trim');
        $this->form_validation->set_rules('CYLAXISLEFT','Cyl Left','trim');
        $this->form_validation->set_rules('CYLAXISRIGHT','Cyl Right','trim');
        $this->form_validation->set_rules('AXISLEFT','Axis Left','trim');
        $this->form_validation->set_rules('AXISRIGHT','Axis Right','trim');
        $this->form_validation->set_rules('ANNUAL','Annual','trim');
        $this->form_validation->set_rules('EXTERNAL_EYE','External Eye','trim');
        $this->form_validation->set_rules('GLAUCOMA','Glaucoma','trim');
        $this->form_validation->set_rules('HEADACHE','Headache / dizzeness','trim');
        $this->form_validation->set_rules('PITY','Pity','trim');
        $this->form_validation->set_rules('RETINA','Retina','trim');
        $this->form_validation->set_rules('CATARACT','Cataract','trim');
        $this->form_validation->set_rules('FB','Laser','trim');
        $this->form_validation->set_rules('LASER','Laser','trim');
        $this->form_validation->set_rules('REFRACTION','Refraction','trim');
        $this->form_validation->set_rules('OTHERS','Others','trim');
        $this->form_validation->set_rules('FRAME','Frame','trim');
        ...
        if ($this->form_validation->run() == TRUE){
            if( $this->m_utility->tokenCheck($this->input->post('TOKEN')) && $this->m_utility->tokenCheck($this->input->post('TOKEN1')) ){
                ...
                $FORM = array(
                    ...
                    'PROCEDURESID'           => $this->input->post('PROCEDURESID'),
                    ...
                    'SPHLEFT'               => $this->input->post('SPHLEFT'),
                    'SPHRIGHT'              => $this->input->post('SPHRIGHT'),
                    'CYLAXISLEFT'           => $this->input->post('CYLAXISLEFT'),
                    'CYLAXISRIGHT'          => $this->input->post('CYLAXISRIGHT'),
                    'AXISLEFT'              => $this->input->post('AXISLEFT'),
                    'AXISRIGHT'             => $this->input->post('AXISRIGHT'),
                    
                    'LATERAL_BORDER_L'      => $this->input->post('LATERAL_BORDER_L'),
                    'MEDICAL_CREASE_L'      => $this->input->post('MEDICAL_CREASE_L'),
                    'TALAR_HEAD_L'          => $this->input->post('TALAR_HEAD_L'),
                    'MIDFOOT_SCORE_L'       => $this->input->post('MIDFOOT_SCORE_L'),
                    'POSTERIOR_CREASE_L'    => $this->input->post('POSTERIOR_CREASE_L'),
                    'EMPTY_HEEL_L'          => $this->input->post('EMPTY_HEEL_L'),
                    'RIGID_EQUINUS_L'       => $this->input->post('RIGID_EQUINUS_L'),
                    'HINDFOOT_SCORE_L'      => $this->input->post('HINDFOOT_SCORE_L'),
                    'TOTAL_SCORE_L'         => $this->input->post('TOTAL_SCORE_L'),
                    'COMPLICATIONS_L'       => $this->input->post('COMPLICATIONS_L'),
                    'TREATMENT_CODE_L'      => $this->input->post('TREATMENT_CODE_L'),
                    'LATERAL_BORDER_R'      => $this->input->post('LATERAL_BORDER_R'),
                    'MEDICAL_CREASE_R'      => $this->input->post('MEDICAL_CREASE_R'),
                    'TALAR_HEAD_R'          => $this->input->post('TALAR_HEAD_R'),
                    'MIDFOOT_SCORE_R'       => $this->input->post('MIDFOOT_SCORE_R'),
                    'POSTERIOR_CREASE_R'    => $this->input->post('POSTERIOR_CREASE_R'),
                    'EMPTY_HEEL_R'          => $this->input->post('EMPTY_HEEL_R'),
                    'RIGID_EQUINUS_R'       => $this->input->post('RIGID_EQUINUS_R'),
                    'HINDFOOT_SCORE_R'      => $this->input->post('HINDFOOT_SCORE_R'),
                    'TOTAL_SCORE_R'         => $this->input->post('TOTAL_SCORE_R'),
                    'COMPLICATIONS_R'       => $this->input->post('COMPLICATIONS_R'),
                    'TREATMENT_CODE_R'      => $this->input->post('TREATMENT_CODE_R'),
                    ...
                    'HMOCHEQUENO'           => $this->input->post('HMOCHEQUENO'),
                    'HMOCHEQUEDATE'         => is_null($this->input->post('HMOCHEQUEDATE')) ? null : date('Y-m-d',strtotime($this->input->post('HMOCHEQUEDATE'))),
                    ...
                    'ANNUAL'                => (Boolean)$this->input->post('ANNUAL') ? 'Y' : 'N',
                    'EXTERNAL_EYE'          => (Boolean)$this->input->post('EXTERNAL_EYE') ? 'Y' : 'N',
                    'GLAUCOMA'              => (Boolean)$this->input->post('GLAUCOMA') ? 'Y' : 'N',
                    'HEADACHE'              => (Boolean)$this->input->post('HEADACHE') ? 'Y' : 'N',
                    'PITY'                  => (Boolean)$this->input->post('PITY') ? 'Y' : 'N',
                    'RETINA'                => (Boolean)$this->input->post('RETINA') ? 'Y' : 'N',
                    'CATARACT'              => (Boolean)$this->input->post('CATARACT') ? 'Y' : 'N',
                    'FB'                    => (Boolean)$this->input->post('FB') ? 'Y' : 'N',
                    'LASER'                 => (Boolean)$this->input->post('LASER') ? 'Y' : 'N',
                    'REFRACTION'            => (Boolean)$this->input->post('REFRACTION') ? 'Y' : 'N',
                ...
                if( $MEDID > 0 ){
                    ...
                    $FORM['UPDATEDBY'] = $this->session->USERID;
                    $FORM['UPDATEDTIME'] = date('Y-m-d H:i:s',time());

                    $this->db->update('medicalrecords', $FORM, array('ID'=>$MEDID));
                    ...
                    $FORM['CREATEDTIME'] = date('Y-m-d H:i:s',time());
                    $FORM['CANCELLED'] = 'N';

                    $this->db->insert('medicalrecords', $FORM);
                ...
                if( $MEDID > 0 ) {
                    ...
                    foreach ($this->input->post('MRLABMONITORING[]') as $key => $v) {
                        if( (int)$v['ID'] > 0 ){

                            $this->db->update('mr_lab_monitoring',array(
                                'LABMONITORINGID'   => $v['LABMONITORINGID'],
                                'LABVALUE'          => $v['LABVALUE'],
                                'UPDATEDBY'         => $this->session->USERID,
                                'UPDATEDTIME'       => date('Y-m-d H:i:s',time()),
                                'CANCELLED'         => (Boolean)$v['CANCELLED'] ? 'Y' : 'N',
                            ...
                            $this->db->insert('mr_lab_monitoring',array(
                                'MEDICALRECORDID'   => $MEDID,
                                'LABMONITORINGID'   => $v['LABMONITORINGID'],
                                'LABVALUE'          => $v['LABVALUE'],
                                'CREATEDBY'         => $this->session->USERID,
                ...
                $data['suc']['MRLABMONITORING'] = $this->MRLABMONITORING($MEDID);
    ...
    private function Previous_NAME($MRID){
        // CREATEDBY SERVED AS DOCTOR ALREADY CHECK AND UPDATED
        // IF NULL MEANS IT JUST PREPARED BY
        $sql = $this->db->query("SELECT PATIENTID, CHECKUPDATE FROM lab_monitoring WHERE ID=? AND CREATEDBY > 0 LIMIT 1", array($MRID))->row();
        if( $sql ){
            $served = $this->db->query("SELECT ID FROM lab_monitoring
                WHERE PATIENTID=? 
                AND DATE(CHECKUPDATE) < ?
                AND CANCELLED='N' ", array(
                    $sql->PATIENTID,
            ...
            foreach ($served as $key => $value) {
                $this->db->update('lab_monitoring',array(
                    'NAME' => ''
                ), array('ID'=> $value->ID));
            }
        }
