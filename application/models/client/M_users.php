...

{
...

	{
		...
		$sql = $this->db->query("SELECT U.ID,U.NAME,U.JOBTITLE, U.POSITION,U.USERNAME,U.CANCELLED,U.ISADDSERVICES,U.ISDISPLAYSERVICESAMOUNT
	...
	public function Form_Data($id) {
		...
		if( (int)$id == 0 ) {
			...
			$data = $this->db->query("SELECT *  FROM users where ID = ? LIMIT 1", array($id))->row();
