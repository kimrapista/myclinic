    ...
    public function My_Account(){
        ...
        if ( isset($this->session->USERID) ){
            ...
            if ($sql->ID > 0) {
                ...
                $users = $this->db->query("SELECT * FROM users WHERE ID=? LIMIT 1",array($sql->ID))->row(); 


                if ( isset($users) ) {

                    $sql->ISADDSERVICES = $users->ISADDSERVICES;

                    $sql->ISDISPLAYSERVICESAMOUNT = $users->ISDISPLAYSERVICESAMOUNT;
                }
