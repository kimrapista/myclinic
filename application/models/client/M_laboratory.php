...
class M_laboratory extends CI_Model
{
	...
	public function Index()
	{
		...
		$sql = $this->db->query("SELECT ID, NAME, TEMPLATE, AUTOADD, CANCELLED FROM laboratory 
			where CLINICID=?
	...
	private function Check_Redundant($ID,$NAME,$CLINICID){
		...
		$sql = $this->db->query("SELECT * FROM laboratory WHERE ID !=? AND NAME=? AND CLINICID=? LIMIT 1",array($ID,$NAME,$CLINICID))->row();

		if( isset($sql) ){
			return 'Laboratory Name is already registered.';
		}
