<!-- ADMIN -->
<section  ng-controller="FormAdvisory">
	<md-progress-linear ng-hide="opt.isLoaded" md-mode="indeterminate"></md-progress-linear>
	<div class="container-xs">
		<form  name="formParent" ng-submit="Submit_Form()" role="form"  >


			<md-card>
				<md-card-content>


					<md-input-container class="md-block">
						<label>Clinic</label>
						<md-select ng-model="form.CLINICID" name="CLINICID"  autofocus>
							<md-option value="0">ALL</md-option>
							<md-option ng-repeat="A in form.CLINICS" ng-value="A.ID" ng-bind="A.CLINICNAME"></md-option>
						</md-select>
						<div ng-messages="formParent.CLINICID.$error" role="alert" md-auto-hide="false">
							<div ng-message="required">This is required.</div>
						</div>
					</md-input-container>


					<md-input-container class="md-block">
						<label>Title</label>
						<input ng-model="form.TITLE"  type="text" >
					</md-input-container>


					<md-input-container class="md-block">
						<label>Message</label>
						<textarea ng-model="form.BODY" name="BODY" md-maxlength="1000" required=""></textarea>
						<div ng-messages="formParent.BODY.$error" role="alert" md-auto-hide="false">
							<div ng-message="required">This is required.</div>
							<div ng-message="md-maxlength">character reach the max limit.</div>
						</div>
					</md-input-container>

					<md-input-container class="md-block">
						<label>Link</label>
						<input ng-model="form.LINK" >
					</md-input-container>


					<div layout="row">
						<md-switch ng-model="form.POST" aria-label="POST" class="md-primary" flex="">
							POST 
						</md-switch>
						<md-input-container flex="">
							<label>POST DATE</label>
							<input ng-model="form.POSTDATE" name="POSTDATE" type="date" ng-disabled="!form.POST"  ng-required="form.POST">
							<div ng-messages="formParent.POSTDATE.$error" role="alert" md-auto-hide="false">
								<div ng-message="required">This is required.</div>
							</div>
						</md-input-container>
						<md-input-container flex="">
							<label>POST TIME</label>
							<input ng-model="form.POSTDATE" name="POSTDATE" type="time" ng-disabled="!form.POST"  ng-required="form.POST">
							<div ng-messages="formParent.POSTDATE.$error" role="alert" md-auto-hide="false">
								<div ng-message="required">This is required.</div>
							</div>
						</md-input-container>
					</div>


					<div layout="column" layout-gt-xs="row" layout-align="end" >
						<md-button ng-disabled="opt.isSubmit  || formParent.$invalid"  type="submit" class="md-raised md-primary">
							<span ng-hide="opt.isSubmit">Save</span>
							<div ng-show="opt.isSubmit" layout="row" layout-align="center">
								<md-progress-circular md-mode="indeterminate" md-diameter="30"></md-progress-circular>
							</div>
						</md-button>

						<md-button href="{{opt.cancelUrl}}"  class=" md-primary" >Cancel</md-button>
					</div>

				</md-card-content>
			</md-card>

		</form>
	</div>
</section>

