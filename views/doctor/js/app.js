...
app.config(function ($routeProvider, $locationProvider) {
    ...
    .when("/clinic", {
        templateUrl: "views/doctor/page_clinic.php"
    })
    .when("/clinic/edit", {
        templateUrl: "views/doctor/form_clinic.php"
    })
    .when("/clinic/subclinic/:P1", {
    ...
    .when("/appointments", {
        templateUrl: "views/doctor/page_appointments.php"
    })
    .when("/sales", {
        templateUrl: "views/doctor/page_sales.php"
    })
    .when("/settings/users", {
        templateUrl: "views/doctor/page_users.php"
    })
    .when("/settings/users/:P1", {
        templateUrl: "views/doctor/form_user.php"
    })
    ...
    .when("/settings/labtemplate", {
        templateUrl: "views/doctor/page_laboratory.php"
    })
    .when("/settings/labtemplate/:P1", {
        templateUrl: "views/doctor/form_laboratory.php"
    })

    .when("/sms", {
        templateUrl: "views/doctor/page_sms.php"
    })
    .when("/sms/:P1", {
        templateUrl: "views/doctor/form_sms.php"
    })
...
app.controller('Navigation', function ($scope, $http, $timeout, $filter, $rootScope, $mdToast, $log, $q, $location, $mdDialog, $mdSidenav, global) {
    ...
    $scope.Check_UserInfo = function () {
        ...
        if ($scope.opt.USER.LEVEL != 'ADMINISTRATOR' && $scope.opt.USER.LEVEL != 'BRANCH ASSISTANT') {
            ...
            if ($scope.opt.USER.SPECIALISTID == 0 || $scope.opt.USER.LICENSENO == '' || $scope.opt.USER.PTR == '') {
                $scope.opt.userWarning = true;
                $location.url('/account');

            } else {
                $scope.opt.userWarning = false;
            }
        }
    }
