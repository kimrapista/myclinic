...
app.controller('FormMedical',function ($scope, $http, $timeout, $filter, $rootScope, $routeParams, $mdDialog, $location,$element, $q, $log, $mdSidenav, global){
		...
		$http.get($scope.opt.formUrl, global.ajaxConfig).then( function(response)  {	
			...
			if( response.data == 'RELOGIN'){
				...
				if( response.data.TOKEN != undefined ){
					...
					if( $scope.opt.EDITMR )	$scope.f.READONLY = false;
					...
					$scope.f.CREATEDBY = parseInt($scope.f.CREATEDBY);
	...
	$scope.Refresh_Services_Discount = function(){
		...
		$http.get($scope.opt.formUrl, global.ajaxConfig).then( function(response)  {	
			...
			if( response.data == 'RELOGIN'){
				...
				if( response.data.TOKEN != undefined ){
					...
					$scope.f.READONLY = parseInt(response.data.READONLY);
					if( $scope.opt.EDITMR )	$scope.f.READONLY = false;

					$scope.f.CREATEDBY = parseInt(response.data.CREATEDBY);

					$scope.f.SERVICES = response.data.SERVICES;
					$scope.f.DISCOUNTS = response.data.DISCOUNTS;
	...
	$scope.Validate_Prescription = function(){

		var invalid = false;

		if ( $scope.f != undefined ) {

			angular.forEach(angular.copy($scope.f.MEDICINES),function(v,k){
				if ( !v.CANCELLED ) {
					if( isNaN(v.MEDICINEID) || $.trim(v.INSTRUCTION) == '' ) invalid = true;
				}
			});
	...
	$scope.Validate_Billing = function(){
		
		var invalid = false;

		if ( $scope.f != undefined ) {

			angular.forEach( angular.copy($scope.f.SERVICES) ,function(v,k){
				if ( !v.CANCELLED ) {
					if( isNaN(v.SERVICEID) || v.QUANTITY < 1 ) invalid = true;
				}
			});


			angular.forEach( angular.copy($scope.f.DISCOUNTS),function(v,k){
				if ( !v.CANCELLED ) {
					if( isNaN(v.DISCOUNTID) || v.AMOUNT < 0 ) invalid = true;
				}
			});
	...
	$scope.Copy_CO_Morbidities = function(event){
		...
		$mdDialog.show(confirm).then(function() {
			...
			if( $scope.f.PREVIOUS != null ){
				$scope.f.COMORBIDITIES = $scope.f.PREVIOUS.COMORBIDITIES;
			}
			else{
	...
	$scope.Copy_Findings = function(event){
		...
		$mdDialog.show(confirm).then(function() {
			...
			if( $scope.f.PREVIOUS != null ){
				$scope.f.FINDINGS = $scope.f.PREVIOUS.FINDINGS;
			}
			else{
				global.Alert('No previous record.');
			}
	...
	$scope.Copy_Diagnosis = function(event){
		...
		$mdDialog.show(confirm).then(function() {
			...
			if( $scope.f.PREVIOUS != null ){
				$scope.f.DIAGNOSIS = $scope.f.PREVIOUS.DIAGNOSIS;
			}
			else{
				global.Alert('No previous record.');
			}
	...
	$scope.Copy_Procedure = function(event){
		...
		$mdDialog.show(confirm).then(function() {
			...
			if( $scope.f.PREVIOUS != null ){
				$scope.f.PROCEDURE_DONE = $scope.f.PREVIOUS.PROCEDURE_DONE;
			}
			else{
				global.Alert('No previous record.');
			}
	...
	$scope.Copy_Instruction = function(event){
		...
		$mdDialog.show(confirm).then(function() {
			...
			if( $scope.f.PREVIOUS != null ){
				$scope.f.INSTRUCTION = $scope.f.PREVIOUS.INSTRUCTION;
			}
			else{
				global.Alert('No previous record.');
			}
	...
	$scope.Copy_Medication = function(event){
		...
		$mdDialog.show(confirm).then(function() {
			...
			if( $scope.f.PREVIOUS != null ){
				$scope.f.MEDICATION = $scope.f.PREVIOUS.MEDICATION;
			}
			else{
				global.Alert('No previous record.');
			}
