...
app.controller('FormService',function ($scope,$http,$timeout,$filter, $location, $rootScope, $routeParams, global){


	$scope.opt = {
		isLoaded : false,
		isSubmit : false,
		formUrl : global.baseUrl + 'settings/services/form-data/',
		submitUrl : global.baseUrl +'settings/services/submit-form',
		cancelUrl : '#!/settings/services'
	}
		...
		$http.post($scope.opt.submitUrl, $scope.form, global.ajaxConfig).then( function(response){
			...
			if( response.data == 'RELOGIN'){
				...
				if( response.data.err != '' ){
					...
					global.Toast('SAVED');
					$location.url('/settings/services');
				}
			}
	...
	if( $routeParams.P1 == 'new' ){
		...
		$scope.opt.formUrl = global.baseUrl + 'settings/services/form-data/0';
		$rootScope.$emit("Title_Global", { TITLE : 'NEW SERVICE', BACKURL: '#!/settings/services' });
	}
	else{
		$scope.opt.formUrl = global.baseUrl + 'settings/services/form-data/' + $routeParams.P1;
		$rootScope.$emit("Title_Global", { TITLE : 'EDIT SERVICE', BACKURL: '#!/settings/services' });
	}
