...
app.factory('LabMonitoringServices',function ($http, $q, $filter, $timeout, $interval, global) {

	var url = global.baseUrl + 'settings/lab_monitoring/index';
	var urlForm = global.baseUrl + 'settings/lab_monitoring/form-data/';
	...
	function Format(detail){
		...
		detail.SORT = parseInt(detail.SORT);
		detail.INDENT = parseInt(detail.INDENT);

		if ( typeof detail.ISACTIVE != 'boolean' ){
			detail.ISACTIVE = detail.ISACTIVE == 'Y' ? true : false;
		}
